...
func _draw_slot(font: Font, slot: Dictionary, origin: Vector2) -> void:
	...
	var border := color if status == "ready" or status == "active" else Color("57546c")
	draw_rect(rect, Color("080812", 0.88), true)
	draw_rect(rect, border, false, 2.0)
	draw_string(font, origin + Vector2(14, 38), str(slot.key), HORIZONTAL_ALIGNMENT_LEFT, -1, 31, border)
	draw_string(font, origin + Vector2(59, 30), str(slot.title), HORIZONTAL_ALIGNMENT_LEFT, -1, 16, Color.WHITE)
	if status == "ready":
		...
		draw_rect(Rect2(origin + Vector2(2, 2), Vector2(141, 90)), Color(0.01, 0.01, 0.04, 0.78), true)
		draw_string(font, origin + Vector2(0, 66), "%.1f" % cooldown_left, HORIZONTAL_ALIGNMENT_CENTER, 145, 27, Color.WHITE)
	else:
		draw_rect(Rect2(origin + Vector2(2, 2), Vector2(141, 90)), Color(0.01, 0.01, 0.04, 0.62), true)
		draw_string(font, origin + Vector2(59, 62), str(slot.requirement), HORIZONTAL_ALIGNMENT_LEFT, 78, 13, Color("9c99af"))
